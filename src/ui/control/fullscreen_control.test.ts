...
describe('FullscreenControl', () => {
    test('appears when fullscreen is enabled', () => {
        ...
        expect(map.getContainer().querySelectorAll('.maplibregl-ctrl-fullscreen-swr')).toHaveLength(1);
    });
    ...
    test('does not appear when fullscreen is not enabled', () => {
        ...
        expect(map.getContainer().querySelectorAll('.maplibregl-ctrl-fullscreen-swr')).toHaveLength(0);
